...
3. **Weather Watchdog:** (Podaci se i dalje primaju u pozadini zbog Modbus kompatibilnosti, ali je UI dio uklonjen radi uštede RAM-a).
...
5. **Clean Mode:** Blokira sav touch input na 60s (koristi `lv_layer_sys()`), sakriva glavnu poruku i prikazuje aktivni tajmer.
...
Glavna navigacija se sada oslanja na **nezavisne ekrane** (beskonačna kružna petlja dugmadima). SwipeContainer je potpuno uklonjen radi brzine:

### 4.1. Ekran 1: Main (Home)
- Prikaz sata, unutrašnje/vanjske temperature i statusnih ikonica.
- Navigacija naprijed vodi na ekran Thermostat.
- Skriveni meni: Long press (5s) na `ui_ButtonHiddenMenu` (gornji lijevi ugao) otvara `ui_PinEntry`.

### 4.2. Ekran 2: Thermostat
- **Setpoint:** Direktna kontrola prevlačenjem (Arc) preko `ui_ArcTemp` (Opseg: 10-40°C).
- **Fan Speed:** Ciklična promjena (Auto → Low → Mid → High) preko srednjeg dugmeta `ui_BtnFan`.
- **Navigacija:** Lijevo dugme vodi nazad na Main, desno dugme vodi na Clean ekran.

### 4.3. Ekran 3: Clean Screen
- **Aktivacija:** Preko `ui_BtnCleanStart` (srednje dugme).
- **Navigacija:** Lijevo dugme vodi nazad na Thermostat, desno dugme vodi nazad na Main (zatvaranje petlje).
